...
/// Coordinates the client, device store, Wake-on-LAN and sleep timer, exposing
/// a small surface for the SwiftUI views. All UI state flows through here so
/// views never touch the transport directly.
///
/// The iOS build omits the macOS-only global keyboard shortcuts.
@MainActor
final class TVControllerViewModel: ObservableObject {
    ...
    init(deviceStore: DeviceStore? = nil) {
        self.deviceStore = deviceStore ?? DeviceStore()
        forwardChanges()
        wireClient()
    }

    /// The device store is a nested ObservableObject; SwiftUI only observes this
    /// view model, so re-publish its changes here — otherwise the device list
    /// and selection never refresh in the UI.
